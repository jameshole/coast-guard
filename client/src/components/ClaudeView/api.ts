...
export async function loadThread(): Promise<Thread> {
  const r = await fetch('/api/claude/thread');
  if (!r.ok) throw new Error(`loadThread ${r.status}`);
  return r.json();
}

export async function resetThread(): Promise<Thread> {
  const r = await fetch('/api/claude/thread/reset', { method: 'POST' });
...
export async function streamMessage(
  ...
  const resp = await fetch('/api/claude/thread/message', {
    method: 'POST',
  ...
  for (;;) {
    ...
    while ((idx = buf.indexOf('\n\n')) >= 0) {
      ...
      buf = buf.slice(idx + 2);
      let channel: 'local' | 'claude' = 'local';
      const dataLines: string[] = [];
      for (const line of block.split('\n')) {
        if (line.startsWith('event:')) {
          const tag = line.slice(6).trim();
          if (tag === 'claude' || tag === 'local') channel = tag;
